...
- Summaries: the Settings tab above Open/Create Vault holds user-specific OpenAI API-key/model configuration outside the Vault. Settings also offers a separate Off/Cheap/Standard/Deep summary budget, used for both automatic capture and manual retry. Off preserves the source without an AI request. Otherwise, configured capture attempts a summary after preserving the source; provider failure leaves the source readable with a retry action. Source text is sent to OpenAI for this optional operation. API requests are bounded, and no provider is required to own or read the archive.
...
Thumbnail preparation runs sequentially outside the desktop command-state lock so selecting paintings can continue during gallery preparation or artwork enrichment. Native command measurements and remaining GUI acceptance are documented in `.scratch/spec-continuation/selection-measurement.md`.
