...
try {
  if (command === "init") {
    ...
    console.log(
      "GG — file-based personal archive.\ninit [areas...] | art|painting|idea URL... [--stdin] | search QUERY | ask QUERY | capture-file FILE | serve | index | probe | queue\n--vault PATH --provider openai|openrouter --model SLUG --config PATH --fixture --focus TEXT --json",
    );
  else {
    const vault = await Vault.open(resolve(values.vault));
    if (command === "index") console.log(await vault.index());
      ...
      if (command === "serve") {
        ...
        const receiver = await createReceiver({
          vault,
          port,
          processCapture: async (capture, signal) =>
            runJob({
              vault,
              config,
        ...
        console.log(
          `GG receiver: ${receiver.url}\nPairing token: ${receiver.token}\nLoad the extension/ folder in Chrome, click GG on a page, and connect using these values.`,
        );
        ...
        if (!positionals.length)
          throw Error("Pass a browser capture JSON file");
        for (const file of positionals) {
          try {
            ...
            const result = await runJob({
              ...
              browserCapture: capture,
              focus: capture.focus,
              ...(values.fixture ? await fixtures() : {}),
            });
            console.log(JSON.stringify(result));
            if (result.exitCode || result.outcome?.status === "failed")
              process.exitCode = 1;
          } catch (error) {
