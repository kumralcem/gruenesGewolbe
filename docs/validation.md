...
- **Attribution research:** several imports retain honest provisional metadata after research failures. Jael and Sisera is a positive checked example: its museum citation supports title/artist/date and the museum image matches the import. One other summary needed a manual correction to avoid guessing a figure’s identity. Automatic identification remains a quality limitation, not a guaranteed result. Twenty saved collection originals were checked byte-for-byte against staging.
