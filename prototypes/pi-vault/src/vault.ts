...
const marker = ".gg-prototype.json";
...
export class Vault {
  ...
  static async create(root: string, areas: string[]) {
    ...
    if ((await readdir(root)).length)
      throw Error("Prototype init requires an empty directory");
    const canonical = await realpath(root);
    await writeFile(
      join(canonical, marker),
      JSON.stringify({ prototype: true, areas }),
      { flag: "wx", mode: 0o600 },
    );
    ...
    return new Vault(canonical, areas);
  }
  static async open(root: string) {
    await plain(root, true);
    await plain(join(root, marker));
    const data = JSON.parse(await readFile(join(root, marker), "utf8"));
    if (
      data.prototype !== true ||
      !Array.isArray(data.areas) ||
      !data.areas.every((a: unknown) => typeof a === "string" && name(a))
    )
      throw Error("Not a prototype vault");
  ...
  async search(query: string): Promise<Hit[]> {
    ...
    for (const { item, path } of await this.items()) {
      ...
      if (item.kind === "idea") {
        try {
          await plain(join(path, "source.txt"));
          content += "\n" + (await readFile(join(path, "source.txt"), "utf8"));
        } catch {
  ...
  save(draft: Draft): Promise<Outcome> {
    const next = this.pending.then(() =>
      this.withWriteLock(() => this.saveOne(draft)),
    );
  ...
  private async saveOne(d: Draft): Promise<Outcome> {
    ...
    if (existing) {
      ...
      await writeFile(temp, recordText(existing.item), { flag: "wx" });
    ...
    try {
      ...
      if (d.kind === "idea")
        await writeFile(join(stage, "source.txt"), d.sourceText!);
      await writeFile(join(stage, "record.md"), recordText(item));
